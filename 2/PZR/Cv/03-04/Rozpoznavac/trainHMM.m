function ret = trainHMM (class_id, train_set, word_features, num_features, word_class, max_iter, max_frames)
% trenovani modelu jedne tridy na datech dane tridy z trenovaci sady
    ...
    % num_features = size (word_features{1}, 1);
    ...
    for n = 1:num_train_class
        word_frames = size(word_features{train_class_set(n)}, 2);
        [num_fps, fts] = splitFramesToStates (word_frames, num_HMM_states);
        num_frames_per_state(n,:) = num_fps;
        frame_to_state (n,1:word_frames) = fts;
    end
    ...
    while (iter <= max_iter)
        ...
        for n = 1:num_train_class
            ...
            for f = 1:word_frames
                ...
                temp = word_features{train_class_set(n)}';
                feature_vectors_in_state(s, count(s),:) = temp(f, :);
                count (s) = count (s) + 1;
            end
        end
        ...
        if (max_iter > 1)  % proved jen kdyz je pozadovano vice iteraci
            ...
            for n = 1:num_train_class
                ...
                word_feat = word_features{train_class_set(n)}';
                [scr(n), path] = computeHMMViterbi_fast (word_feat, word_frames, class_id);
                frame_to_state (n,1:word_frames) = path;  % mame prirazeni framu ke stavum
                for s = 1:num_HMM_states
